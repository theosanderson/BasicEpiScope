...
difference(){
    union(){
        difference(){
            hull(){
                 translate([0,0,18])  # cylinder(r=22,h=0.01);
    translate([0,0,14])  # cylinder(r=26,h=0.01);
...
hull(){
    translate([0,0,100+vert])
difference(){

cylinder(r=hole_diam/2,h=20);
    cylinder(r=hole_diam/2-2,h=20);
    
      translate([0,10,0]) cube([90,55,90],center=true);
    
};

#difference(){
    
    cylinder(r=rms_radius()+5,h=15);
    cylinder(r=rms_radius()+2,h=15);
  translate([0,15,0])  cube([40,45,40],center=true);
...
translate([0,0,15])
    cylinder(r=rms_radius()+2,h=15);
       translate([0,0,100+vert]) cylinder(r=hole_diam/2-2,h=20);
}
...
translate([0,0,120+vert]){
    
    difference(){
        
        cube([64,85,2],center=true);
